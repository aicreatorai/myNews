...
2. **输出 3-5 条**（优先 5 条），每条标注真实来源
3. **全量搜索**所有关键词：Android OS、Kotlin/Jetpack、Android Studio、厂商定制层、KMP
...
- **标题格式**：`【Android事件+核心变化+版本号/数据】`
- **每条结构**：标题→导语→深度报道（更新全貌/代码要点/生态影响）→来源
- **代码/配置示例必填**：每条至少有一段 Kotlin/XML/Gradle 代码
...
| **输出路径** | `{workspace}/news/{YYYYMM}/{YYYYMMDD}/08_Android开发生态.md` |
...
> **⚠️ 核心原则**：Android 开发生态是早间新闻的专业模块，必须优先选择 `{DATE-7} 07:00` 至 `{DATE} 07:00` 期间发生的最新动态。先搜索，再写作，严禁凭记忆生成内容。
