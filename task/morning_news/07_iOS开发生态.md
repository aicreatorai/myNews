...
2. **输出 7-15 条**（优先 15 条），每条标注真实来源
3. **全量搜索**：P0(Swift/SwiftUI/Xcode) → P1(App Store/visionOS/Apple Intelligence) → P2(iOS新功能/并发模型)
4. **每条需包含具体技术细节**，禁止空洞陈述
...
- **标题格式**：`【iOS/Apple事件+核心变化+技术要点】`
- **每条结构**：标题→导语→深度报道→代码/配置要点→来源
- **代码示例必填**：每条至少有一段 Swift/SwiftUI 代码示例
- **技术细节必填**：每条必须有具体 API/框架名称和版本号
...
| **输出路径** | `{workspace}/news/{YYYYMM}/{YYYYMMDD}/07_iOS开发生态.md` |
...
> **⚠️ 核心原则**：iOS 开发生态是早间新闻的专业模块，必须优先选择 `{DATE-7} 07:00` 至 `{DATE} 07:00` 期间发生的最新动态。先搜索，再写作，严禁凭记忆生成内容。
