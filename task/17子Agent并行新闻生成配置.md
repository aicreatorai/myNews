...
> **说明**：本配置文件定义如何通过 22 个子 Agent，并行执行 `task/morning_news/` 目录下的 22 个新闻板块任务，一次性生成完整早间新闻。
...
以下规则适用于 **Agent 13（AI知识点）、Agent 18（AI工具使用）、Agent 20（AI+教育）、Agent 21（个人提升）、Agent 22（AI产品经理）** 共 5 个知识/学习类模块：
...
#### 规则二：主题轮换制（7 天一轮换，每天只写 2-3 个子领域）

知识类模块不再每天覆盖全部子领域，改为 **每天聚焦 2-3 个子领域**，按周循环：
...
| 周日 | 组 G | 综合回顾+本周热点知识点精选混编 |

- 每天只写聚焦子领域下的知识点，其他子领域即使有内容也**不写**
- 若当天聚焦子领域搜索后无新热点，可**横向扩展到相邻子领域**（如组A没内容→扩展到组B），但需在文件头部标注"本期扩展覆盖"
...
| 周三 | 组 C | 音频音乐类 + 视频类 |
| 周四 | 组 D | 办公文档类 + 学习教育类 |
| 周五 | 组 E | 自动化工作流类 + 会议沟通类 |
| 周六 | 组 F | 国产AI工具专题 + 新兴工具 |
| 周日 | 组 G | 本周热门工具回顾 + 读者提问解答 |
...
|------|--------|---------|
| 周一 | 组 A | 政策动态 + K12学科教学 |
| 周二 | 组 B | AI+教育落地案例 + 大学/职教 |
| 周三 | 组 C | 低成本创意方案 + 语言学习 |
| 周四 | 组 D | 学科教学（语数英科） + 素质教育 |
| 周五 | 组 E | 家庭教育 + 自习备考 |
| 周六 | 组 F | 职业教育 + 终身学习 |
| 周日 | 组 G | 本周灵感汇总 + 教育工具速览 |
...
> **执行说明**：日期按星期计算（如 2026-05-14 周四→组 D）。各模块的任务文件中已有的搜索关键词分类与轮换表对应，Agent 只需根据当天星期几选择对应的子领域关键词进行搜索，**其他子领域的关键词跳过不搜**。
...
- 若当天聚焦子领域在历史记录中已全部达到 L4（无可写新内容），且相邻子领域也无新热点：
  - 13_AI知识点：可降至 **3-5 条高质量深度条目**，避免凑数
  - 18_AI工具使用：可降至 **5-8 条**，优先写有新更新的工具
  - 20_AI+教育：可降至 **3-5 条**，必须有真实新案例
  - 21_个人提升：可降至 **3-5 条**，必须有新方法或新研究
  - 22_AI产品经理：可降至 **3-5 条**，必须有新案例或新方法论
...
- 知识类模块（13/18/20/21/22）的搜索窗口统一为 **`{DATE-7} 07:00 ~ {DATE} 07:00`（7天）**，而非新闻类模块的 24 小时
...
| **知识条目数** | 3-20 条（遵循时序去重规则五弹性调整，优先不重复；当日有丰富新热点则15-20条，聚焦子领域无新内容则3-5条高质量深度条目，宁缺毋滥） |
...
| **输出文件** | `news/202605/20260514/18_AI工具使用.md` |
| **新闻条数** | 5-15 条（遵循时序去重规则五弹性调整，优先不重复；类别内有新更新的工具则10-15条，否则5-8条，宁缺毋滥） |
...
| **输出文件** | `news/202605/20260514/19_商业思维与AI变现.md` |
| **新闻条数** | 5-10 条（优先生成 10 条） |
...
| **输出文件** | `news/202605/20260514/20_AI+教育落地与灵感.md` |
| **新闻条数** | 3-10 条（遵循时序去重规则五弹性调整，优先不重复；当日有新政策/新案例则5-10条，否则3-5条，宁缺毋滥） |
| **覆盖范围** | AI+教育政策动态、落地案例、低成本方案、创意灵感、学科教学AI、职业教育AI、家庭教育AI |
| **定位** | 面向教育从业者和创业者的 AI 落地灵感源，聚焦小成本、可落地的教育 AI 方案 |
...
| **输出文件** | `news/202605/20260514/21_个人提升.md` |
| **新闻条数** | 3-10 条（遵循时序去重规则五弹性调整，优先不重复；当日有新方法/新研究则5-10条，否则3-5条，宁缺毋滥） |
...
| **输出文件** | `news/202605/20260514/22_AI产品经理.md` |
| **新闻条数** | 3-10 条（遵循时序去重规则五弹性调整，优先不重复；当日有新案例/新方法论则5-10条，否则3-5条，宁缺毋滥） |
...
### 第二步：并行启动 20 个子 Agent

使用 `task` 工具的 Team Mode，同时启动所有 20 个子 Agent。每个 Agent 的 prompt 内容为对应任务文件的完整内容（日期变量已填充）。
...
| 13 | `news/202605/20260514/13_AI知识点.md` | 3-20 条（聚焦子领域新热点多则15-20，无新内容则3-5高质量深度） |
...
| 18 | `news/202605/20260514/18_AI工具使用.md` | 5-15 条（轮换类别有新更新则10-15，否则5-8） |
| 19 | `news/202605/20260514/19_商业思维与AI变现.md` | 5-10 条 |
| 20 | `news/202605/20260514/20_AI+教育落地与灵感.md` | 3-10 条（有新政策/案例则5-10，否则3-5） |
| 21 | `news/202605/20260514/21_个人提升.md` | 3-10 条（有新方法/研究则5-10，否则3-5） |
| 22 | `news/202605/20260514/22_AI产品经理.md` | 3-10 条（有新案例/方法论则5-10，否则3-5） |
...
### 🔄 失败重试策略
1. **全量搜索不变**：重试时使用与首次执行完全相同的搜索关键词列表（知识类模块使用轮换组内的相同关键词），**不得减少**
2. **max_turns 设置**：
   ...
   - **知识类模块（轮换后）**：因每天只搜2-3个子领域，max_turns下调：
     ...
     - 20_AI+教育 / 21_个人提升 / 22_AI产品经理：**60**（原100，轮换后搜索量大幅减少）
...
1. **推荐分 3 批（均匀分配重量级 Agent）**：
   ...
   - **第三批（9 个）**：04 软件开发、05 开发语言、09 跨平台开发生态、10 移动端生态、14 产品发布、**19 商业思维与AI变现**、**20 AI+教育落地与灵感**、**21 个人提升**、**22 AI产品经理**
